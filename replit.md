...
- July 28, 2025: ✅ ADMIN LOGIN CREDENTIALS FIX: Fixed authentication issues for both admin@canva.com and shams.aranib@canva.com users. Updated middleware to properly handle snake_case database field names (is_admin, role_type) instead of camelCase. Fixed role_type for admin@canva.com from empty string to 'admin'. Set admin@canva.com password to 'admin123' and shams.aranib@canva.com works with 'password123'. Extended JWT token expiry to 7 days to reduce frequent authentication failures. Both users now have proper admin access with working login credentials.
