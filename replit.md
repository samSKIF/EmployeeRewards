...
- July 24, 2025: Successfully implemented complete recognition module toggle functionality in corporate management dashboard. Fixed critical database schema issues (renamed feature_name to feature_key column, added missing enabled_at and enabled_by columns). Created backend API endpoints for organization feature management (/api/management/organizations/{id}/features/{featureKey}) and user-facing feature checking (/api/admin/organization/features). Updated useOrganizationFeatures hook to fetch real feature data from database. Recognition toggle now works end-to-end: when ON, toggle turns green and Recognition & Rewards section appears in admin menu; when OFF, toggle turns gray and section disappears completely. Corporate admins can now control recognition module activation per organization through the subscription management interface.
