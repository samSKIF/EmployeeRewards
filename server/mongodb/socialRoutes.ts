  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const postData = {
        ...
        pollOptions:
          type === 'poll' && pollOptions ? JSON.parse(pollOptions) : undefined,
        isPinned: false,
      };
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || 'Failed to fetch post' });
    }
  }
);
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (
        !type ||
        !['like', 'love', 'celebrate', 'support', 'insightful'].includes(type)
      ) {
        return res.status(400).json({ message: 'Invalid reaction type' });
      }

      const success = await socialService.addReactionToPost(
        id,
        req.user.id,
        req.user.name,
        type
      ...
      res
        .status(500)
        .json({ message: error.message || 'Failed to add reaction' });
    }
  }
);
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const success = await socialService.removeReactionFromPost(
        id,
        req.user.id
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || 'Failed to fetch social stats' });
    }
  }
);
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || 'Failed to search posts' });
    }
  }
);
