...
async function runMigration() {
  console.log("Starting database migration for three-tier account structure...");
  ...
  try {
    ...
    if (checkRoleTypeColumn.rows.length === 0) {
      ...
      `);
      console.log("Added role_type, organization_id, created_by, and permissions columns to users table");
    } else {
    ...
    try {
      ...
      console.warn("Could not create products_seller_id_idx index:", indexError.message);
    }
