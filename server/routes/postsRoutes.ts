...
import { db } from "../db";
import { interestChannelPosts, interestChannelPostLikes, interestChannelPostComments, users } from "../../shared/schema";
...
router.post("/:id/like", verifyToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (existingLike.length > 0) {
      // Unlike the post
      await db.delete(interestChannelPostLikes)
        .where(and(
          eq(interestChannelPostLikes.postId, postId),
          eq(interestChannelPostLikes.userId, userId)
        ));
...
router.post("/:id/comments", verifyToken, async (req: AuthenticatedRequest, res) => {
  try {
    const postId = parseInt(req.params.id);
    const userId = req.user?.id;
    ...
    // Create the comment
    const [comment] = await db.insert(interestChannelPostComments)
      .values({
        postId,
        authorId: userId,
