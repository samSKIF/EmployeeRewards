...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      try {
        ...
        if (currentUserId !== employeeId) {
          return res
            .status(403)
            .json({
              message: "Not authorized to update this employee's interests",
            });
        }
        ...
        await db.transaction(async (tx) => {
          await tx.insert(employeeInterests).values({
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: 'Failed to fetch available interests' });
      }
    }
  );
      ...
      try {
        ...
        const celebrations = [
          ...
          ...anniversaryUsers.map((employee) => {
            const years = employee.dateJoined
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        for (let i = 1; i <= 5; i++) {
          ...
          celebrations.push(
            ...
            ...anniversaryUsers.map((user) => {
              const years = user.hireDate
                ? targetDate.getFullYear() -
                  new Date(user.hireDate).getFullYear()
                : 0;
              return {
                id: user.id,
                user: {
        ...
        res
          .status(500)
          .json({ error: 'Failed to fetch upcoming celebrations' });
      }
    }
  );

  app.get(
    '/api/celebrations/extended',
    verifyToken,
    async (req: AuthenticatedRequest, res) => {
      try {
        const { department, location } = req.query;
        console.log('Extended celebrations API called with filters:', {
          department,
          location,
        });
        const celebrations = [];
        ...
        for (let i = -5; i <= 5; i++) {
          ...
          if (department && department !== 'all') {
            ...
            anniversaryConditions.push(
              eq(users.department, department as string)
            );
          }
        ...
        res
          .status(500)
          .json({ error: 'Failed to fetch extended celebrations' });
      }
    }
  );
  ...
  app.post('/api/auth/register', async (req, res) => {
    try {
      ...
      if (firebaseUid && firebaseUser) {
        ...
        console.log(
          'Standard registration successful for:',
          userWithoutPassword
        );
  ...
  app.post('/api/auth/login', async (req, res) => {
    try {
      ...
      const passwordMatch = await storage.verifyPassword(
        password,
        user.password
      );
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: error.message || 'Failed to award points' });
      }
    }
  );
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: error.message || 'Failed to get balance' });
      }
    }
  );
  ...
  app.post('/api/users/metadata', async (req, res) => {
    try {
      ...
      if (existingUser) {
        console.log(
          'User already exists, updating Firebase UID',
          existingUser.id
        );
      ...
      res
        .status(500)
        .json({ message: error.message || 'Failed to save user metadata' });
    }
  });
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const duplicateCheck = await storage.checkDuplicateUser(
          email,
          name,
          surname
        );
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        console.log(
          `Returning ${usersWithBalance.length} users for company ${companyId || 'unknown'}`
        );
  ...
  app.post(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        res
          .status(500)
          .json({ message: 'Failed to update shop configuration' });
      }
    }
  );
      ...
      try {
        ...
        const giftCardProducts = [
          {
            ...
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1584990451792-a664249664bc?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
          ...
          },
          {
            ...
            description:
              '$35 Uber Eats credit for meals delivered to your door.',
            category: 'Gift Cards',
            points: 280,
            imageUrl:
              'https://images.unsplash.com/photo-1593504049359-74330189a345?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '$25 Apple App Store credit for apps, games and entertainment.',
            category: 'Gift Cards',
            points: 200,
            imageUrl:
              'https://images.unsplash.com/photo-1585184394271-4c0a47dc59c9?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '$25 Google Play credit for apps, games, books, and more.',
            category: 'Gift Cards',
            points: 200,
            imageUrl:
              'https://images.unsplash.com/photo-1611944212129-29977ae1398c?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '$30 DoorDash credit for food delivery from your favorite restaurants.',
            category: 'Gift Cards',
            points: 240,
            imageUrl:
              'https://images.unsplash.com/photo-1582060371588-5d30bf398aa1?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 300,
            imageUrl:
              'https://images.unsplash.com/photo-1614680376573-df3480f0c6ff?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '$50 Target gift card for shopping essentials and more.',
            category: 'Gift Cards',
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1580828343064-fde4fc206bc6?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1601524909162-ae8725290836?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '$100 Best Buy gift card for electronics and appliances.',
            category: 'Gift Cards',
            points: 800,
            imageUrl:
              'https://images.unsplash.com/photo-1593784991095-a205069470b6?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          ...
          },
          {
            ...
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1609092472326-41329e320fbc?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 480,
            imageUrl:
              'https://images.unsplash.com/photo-1621259182978-fbf93132d53d?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 480,
            imageUrl:
              'https://images.unsplash.com/photo-1607853202273-797f1c22a38e?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '$75 Home Depot gift card for home improvement projects.',
            category: 'Gift Cards',
            points: 600,
            imageUrl:
              'https://images.unsplash.com/photo-1578496479531-32e296d5c6e1?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 600,
            imageUrl:
              'https://images.unsplash.com/photo-1516822669470-73637e892be3?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '$50 Sephora gift card for beauty and skincare products.',
            category: 'Gift Cards',
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1576426863848-c21f53c60b19?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 320,
            imageUrl:
              'https://images.unsplash.com/photo-1555992336-fb0d29498b13?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 800,
            imageUrl:
              'https://images.unsplash.com/photo-1539183204366-63a0589187ab?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 450,
            imageUrl:
              'https://images.unsplash.com/photo-1604913571179-f9642e6b43f8?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 420,
            imageUrl:
              'https://images.unsplash.com/photo-1580543687419-070d3bd4858c?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 450,
            imageUrl:
              'https://images.unsplash.com/photo-1520342868574-5fa3804e551c?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1534723452862-4c874018d66d?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
        ...
        const electronicsProducts = [
          {
            ...
            points: 650,
            imageUrl:
              'https://images.unsplash.com/photo-1607083206968-13611e3d76db?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          ...
          },
          {
            ...
            points: 500,
            imageUrl:
              'https://images.unsplash.com/photo-1546868871-7041f2a55e12?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 180,
            imageUrl:
              'https://images.unsplash.com/photo-1603539444875-76e7684265f6?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          ...
          },
          {
            ...
            description:
              'Voice-controlled smart speaker with virtual assistant.',
            category: 'Electronics',
            points: 350,
            imageUrl:
              'https://images.unsplash.com/photo-1549482199-bc1ca6f58502?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          ...
          },
          {
            ...
            points: 250,
            imageUrl:
              'https://images.unsplash.com/photo-1587047163886-e71b96567eb9?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 350,
            imageUrl:
              'https://images.unsplash.com/photo-1623515651673-28033bd10d13?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 280,
            imageUrl:
              'https://images.unsplash.com/photo-1569073120512-05362a6b92e4?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1540885762261-a2ca01f290f9?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 450,
            imageUrl:
              'https://images.unsplash.com/photo-1595781572981-d63151b232ed?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            name: 'Smart Scale',
            description:
              'Digital bathroom scale with health metrics and app integration.',
            category: 'Electronics',
            points: 320,
            imageUrl:
          ...
          },
          {
            ...
            description:
              'Foldable Bluetooth keyboard for tablets and smartphones.',
            category: 'Electronics',
            points: 200,
            imageUrl:
              'https://images.unsplash.com/photo-1516317518460-4a16985740e6?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 550,
            imageUrl:
              'https://images.unsplash.com/photo-1558002038-1055e2fc65af?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 480,
            imageUrl:
              'https://images.unsplash.com/photo-1567769541735-d43de3326c00?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 150,
            imageUrl:
              'https://images.unsplash.com/photo-1605773527852-c546a8584ea3?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 220,
            imageUrl:
              'https://images.unsplash.com/photo-1634328783781-b542bce0b3a6?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 380,
            imageUrl:
              'https://images.unsplash.com/photo-1612815154858-60aa4c59eaa6?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          ...
          },
          {
            ...
            points: 220,
            imageUrl:
              'https://images.unsplash.com/photo-1633060284626-89f955be6f0a?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Bluetooth gaming controller compatible with PC and mobile.',
            category: 'Electronics',
            points: 280,
            imageUrl:
              'https://images.unsplash.com/photo-1580327344181-c1163234e5a0?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Set of 4 Bluetooth trackers for keys, wallets, and more.',
            category: 'Electronics',
            points: 190,
            imageUrl:
              'https://images.unsplash.com/photo-1513116476489-7635e79feb27?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 700,
            imageUrl:
              'https://images.unsplash.com/photo-1507582020474-9a35b7d455d9?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Instant camera with built-in printer for immediate photos.',
            category: 'Electronics',
            points: 650,
            imageUrl:
        ...
        const experiencesProducts = [
          {
            ...
            points: 550,
            imageUrl:
              'https://images.unsplash.com/photo-1542282088-72c9c27ed0cd?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            name: 'Movie Tickets',
            description:
              'Two premium movie tickets for the theater of your choice.',
            category: 'Experiences',
            points: 150,
            imageUrl:
          ...
          },
          {
            ...
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1506377247377-2a5b3b417ebb?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 350,
            imageUrl:
              'https://images.unsplash.com/photo-1507048331197-7d4ac70811cf?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Two tickets to a live music performance of your choice.',
            category: 'Experiences',
            points: 500,
            imageUrl:
              'https://images.unsplash.com/photo-1501612780327-45045538702b?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '12-month membership to local art and science museums.',
            category: 'Experiences',
            points: 600,
            imageUrl:
              'https://images.unsplash.com/photo-1503632235181-2618281d021e?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 800,
            imageUrl:
              'https://images.unsplash.com/photo-1507608869274-d3177c8bb4c7?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 300,
            imageUrl:
              'https://images.unsplash.com/photo-1565122640447-3128631baa36?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 450,
            imageUrl:
              'https://images.unsplash.com/photo-1511098217401-2291d3b7cebd?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Two tickets to a popular theme park for a full day of fun.',
            category: 'Experiences',
            points: 700,
            imageUrl:
              'https://images.unsplash.com/photo-1543313661-988f8be8809a?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 380,
            imageUrl:
              'https://images.unsplash.com/photo-1543101516-5bcc9614f918?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Exhilarating zip-line course through scenic landscapes.',
            category: 'Experiences',
            points: 550,
            imageUrl:
              'https://images.unsplash.com/photo-1544230980-8f19ebdb9ea1?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 650,
            imageUrl:
              'https://images.unsplash.com/photo-1587174486073-ae5e5cff23aa?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Indoor skydiving experience with professional instruction.',
            category: 'Experiences',
            points: 480,
            imageUrl:
              'https://images.unsplash.com/photo-1511169355326-be606c6e1da7?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 250,
            imageUrl:
              'https://images.unsplash.com/photo-1527224857830-43a7acc85260?w=500&h=300&fit=crop',
            supplier: 'tillo',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Behind-the-scenes tour with beer tasting flight for two.',
            category: 'Experiences',
            points: 300,
            imageUrl:
              'https://images.unsplash.com/photo-1559526324-593bc073d938?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Indoor rock climbing experience with gear and instruction.',
            category: 'Experiences',
            points: 280,
            imageUrl:
          ...
          },
          {
            ...
            description:
              '15-minute helicopter sightseeing tour for one person.',
            category: 'Experiences',
            points: 1200,
            imageUrl:
              'https://images.unsplash.com/photo-1583991111178-7b042e3077f6?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Beginner horseback riding lesson with professional instruction.',
            category: 'Experiences',
            points: 350,
            imageUrl:
              'https://images.unsplash.com/photo-1511195448591-062cec834bc2?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 320,
            imageUrl:
              'https://images.unsplash.com/photo-1547333590-47fae5f58d21?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1547048615-da56eb92d444?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 370,
            imageUrl:
              'https://images.unsplash.com/photo-1583623025817-d180a2fe075e?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '1-hour axe throwing session with instruction for two people.',
            category: 'Experiences',
            points: 280,
            imageUrl:
              'https://images.unsplash.com/photo-1574103188526-4faae477d34e?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Guided nighttime astronomy experience with telescopes.',
            category: 'Experiences',
            points: 420,
            imageUrl:
              'https://images.unsplash.com/photo-1509773896068-7fd415d91e2e?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
        ...
        const wellnessProducts = [
          ...
          },
          {
            ...
            points: 250,
            imageUrl:
              'https://images.unsplash.com/photo-1600334129128-685c5582fd35?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              '8-week guided meditation program with certified instructor.',
            category: 'Wellness',
            points: 400,
            imageUrl:
              'https://images.unsplash.com/photo-1536623975707-c4b3b2af565d?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Essential oil diffuser with 6 premium essential oils.',
            category: 'Wellness',
            points: 320,
            imageUrl:
              'https://images.unsplash.com/photo-1608571423902-eed4a5ad8108?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 380,
            imageUrl:
              'https://images.unsplash.com/photo-1631756964162-25c8c07579b1?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Traditional acupuncture therapy with certified practitioner.',
            category: 'Wellness',
            points: 450,
            imageUrl:
              'https://images.unsplash.com/photo-1545205597-3d9d02c29597?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 220,
            imageUrl:
              'https://images.unsplash.com/photo-1598550593506-b035cba3ba0e?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Personalized nutrition consultation with registered dietitian.',
            category: 'Wellness',
            points: 600,
            imageUrl:
              'https://images.unsplash.com/photo-1505576633757-0ac1084f63cd?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 300,
            imageUrl:
              'https://images.unsplash.com/photo-1542728928-1413d1894ed1?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 280,
            imageUrl:
              'https://images.unsplash.com/photo-1576095910607-644cd66e5c65?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 150,
            imageUrl:
              'https://images.unsplash.com/photo-1615310748514-99796d46e94e?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 200,
            imageUrl:
              'https://images.unsplash.com/photo-1539207554081-7214fbd6e282?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 420,
            imageUrl:
              'https://images.unsplash.com/photo-1595238242018-22219945cf66?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 350,
            imageUrl:
              'https://images.unsplash.com/photo-1546483667-f62d66b96636?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 180,
            imageUrl:
              'https://images.unsplash.com/photo-1563911892437-1feda0179e41?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 480,
            imageUrl:
              'https://images.unsplash.com/photo-1598803783347-dada77361e66?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          ...
          },
          {
            ...
            description:
              'Adjustable brace for improved posture and back support.',
            category: 'Wellness',
            points: 140,
            imageUrl:
              'https://images.unsplash.com/photo-1537344836915-25a58b04e69c?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 230,
            imageUrl:
              'https://images.unsplash.com/photo-1532771522233-9079b9616c44?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'White noise machine with natural sounds for better sleep.',
            category: 'Wellness',
            points: 250,
            imageUrl:
              'https://images.unsplash.com/photo-1631703412785-e9754dea55c6?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Set of 3 air-purifying houseplants with decorative pots.',
            category: 'Wellness',
            points: 280,
            imageUrl:
              'https://images.unsplash.com/photo-1545165375-7c5f3a1a2c83?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Deep tissue massage foam roller and balls for recovery.',
            category: 'Wellness',
            points: 210,
            imageUrl:
              'https://images.unsplash.com/photo-1600881333168-2ef49b341f30?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            description:
              'Targeted cold therapy for muscle recovery and pain relief.',
            category: 'Wellness',
            points: 370,
            imageUrl:
              'https://images.unsplash.com/photo-1605296867424-35c82a8b1da1?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
          {
            ...
            points: 320,
            imageUrl:
              'https://images.unsplash.com/photo-1614255976202-43b92dfb7d67?w=500&h=300&fit=crop',
            supplier: 'carlton',
            isActive: true,
            createdBy: req.user.id,
          },
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (!name || !channelType) {
          return res
            .status(400)
            .json({ message: 'Channel name and type are required' });
        }
        ...
        const [newChannel] = await db
          ...
          .values({
            ...
            accessLevel,
            allowedDepartments:
              allowedDepartments.length > 0 ? allowedDepartments : null,
        ...
        if (
          autoAddMembers &&
          (allowedDepartments.length > 0 || allowedLocations.length > 0)
        ) {
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const trendingChannels = await db
          .select()
      ...
      try {
        ...
        if (!membership) {
          return res
            .status(403)
            .json({
              message: 'You must be a member of this space to create posts',
            });
        }
  ...
  app.get(
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        const channelsWithPosts = await db
          .select({
          ...
          .leftJoin(
            interestChannelPosts,
            eq(interestChannels.id, interestChannelPosts.channelId)
          )
    ...
    async (req: AuthenticatedRequest, res) => {
      try {
        ...
        if (existingMember.length > 0) {
          return res
            .status(400)
            .json({ message: 'Already a member of this channel' });
        }
        ...
        if (channel.accessLevel === 'approval_required') {
          ...
          return res
            ...
            .json({
              message:
                'This channel is invite-only. You need an invitation to join.',
            });
        } else {
      ...
      try {
        ...
        if (
          ...
          return res
            .status(403)
            .json({
              message: 'Only channel creators and admins can add new admins',
            });
        }

        // Update user role to admin
        await db
          .update(interestChannelMembers)
          .set({ role: 'admin' })
          .where(
            and(
              eq(interestChannelMembers.channelId, channelId),
      ...
      try {
        ...
        if (channel.createdBy !== req.user.id && req.user.id !== userId) {
          return res
            ...
            .json({
              message:
                'Only channel creators can remove admins, or admins can remove themselves',
            });
        }
      ...
      try {
        ...
        if (
          ...
          return res
            ...
            .json({
              message:
                'Only channel creators and admins can view join requests',
            });
        }
  ...
  app.patch(
    '/api/channels/:id/join-requests/:requestId',
    verifyToken,
    async (req: AuthenticatedRequest, res) => {
      try {
        if (!req.user) {
        ...
        if (
          ...
          return res
            ...
            .json({
              message:
                'Only channel creators and admins can approve join requests',
            });
        }
      ...
      try {
        ...
        if (
          ...
          return res
            .status(403)
            .json({
              message: 'Only channel creators and admins can remove members',
            });
        }
        ...
        if (userId === channel.createdBy) {
          return res
            .status(400)
            .json({ message: 'Cannot remove channel creator' });
        }
