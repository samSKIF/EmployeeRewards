...
import { getTenantDb, getCompanyIdFromUser, getCompanyIdFromEmail } from '../tenant-db';
 ...
 */
export async function tenantRouting(req: TenantRequest, res: Response, next: NextFunction) {
  try {
    const user = req.user;
 ...
 */
export function ensureTenantAccess(req: TenantRequest, res: Response, next: NextFunction) {
  const user = req.user;
