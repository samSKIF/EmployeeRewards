...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/employees/:id/interests', verifyToken, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      res.status(500).json({ message: "Failed to fetch employee interests", error: error.message });
    }
  });
  ...
  app.post('/api/employees/:id/interests', verifyToken, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      await db.delete(employeeInterests).where(eq(employeeInterests.employeeId, employeeId));
      ...
      res.status(500).json({ message: "Failed to update employee interests", error: error.message });
    }
  });
