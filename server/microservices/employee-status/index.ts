...
router.get("/status-types", async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user?.isAdmin) {
      return res.status(403).json({ message: "Not authorized to view status types" });
    }
    ...
    res.status(500).json({ message: error.message || "Failed to fetch status types" });
  }
});
...
router.post("/status-types", async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user?.isAdmin) {
      return res.status(403).json({ message: "Not authorized to create status types" });
    }
    ...
    res.status(500).json({ message: error.message || "Failed to create status type" });
  }
});
...
router.patch("/status-types/:id", async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user?.isAdmin) {
      return res.status(403).json({ message: "Not authorized to update status types" });
    }
    ...
    res.status(500).json({ message: error.message || "Failed to update status type" });
  }
});
...
router.delete("/status-types/:id", async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user?.isAdmin) {
      return res.status(403).json({ message: "Not authorized to delete status types" });
    }
    ...
    if (statusType.isSystem) {
      return res.status(403).json({ message: "Cannot delete system status types" });
    }
    ...
    res.status(500).json({ message: error.message || "Failed to delete status type" });
  }
});

// Get active employee statuses (for display)
router.get("/users/:userId/statuses", async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const currentStatuses = await db
      ...
      .innerJoin(employeeStatusTypes, eq(employeeStatuses.statusTypeId, employeeStatusTypes.id))
      .where(
        and(
    ...
    res.status(500).json({ message: error.message || "Failed to fetch employee statuses" });
  }
});
...
router.post("/assign-status", async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user?.isAdmin) {
      return res.status(403).json({ message: "Not authorized to assign statuses" });
    }
    ...
    let endDate = validatedData.endDate ? new Date(validatedData.endDate) : null;
    ...
    res.status(500).json({ message: error.message || "Failed to assign status" });
  }
});
...
router.delete("/statuses/:id", async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user?.isAdmin) {
      return res.status(403).json({ message: "Not authorized to remove statuses" });
    }
    ...
    // Delete the status
    await db
      .delete(employeeStatuses)
      .where(eq(employeeStatuses.id, statusId));
    
    res.status(200).json({ message: "Status removed successfully" });
  } catch (error: any) {
    console.error("Error removing status:", error);
    res.status(500).json({ message: error.message || "Failed to remove status" });
  }
});
...
router.post("/batch-assign", async (req: AuthenticatedRequest, res) => {
  try {
    if (!req.user?.isAdmin) {
      return res.status(403).json({ message: "Not authorized to batch assign statuses" });
    }
    ...
    res.status(500).json({ message: error.message || "Failed to batch assign statuses" });
  }
});
