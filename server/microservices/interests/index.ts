  ...
  try {
    ...
    if (query && typeof query === 'string') {
      ...
      try {
        ...
        const result = await db.select().from(interests).limit(10);
      ...
      allInterests = await db.select().from(interests).limit(20);
    }
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
        ...
        .where(
          and(
            ...
            or(
              ...
              isCurrentUser
                ? eq(employeeInterests.visibility, employeeInterests.visibility)
                : undefined,
      ...
      res
        .status(500)
        .json({
          message: error.message || 'Failed to fetch employee interests',
        });
    }
  }
);
...
const updateInterestsSchema = z.array(
  z.object({
...
router.post(
  '/employees/:id/interests',
  validateRequestBody(updateInterestsSchema),
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (currentUserId !== employeeId) {
        return res
          .status(403)
          .json({
            message: "Not authorized to update this team member's interests",
          });
      }
      ...
      await db.transaction(async (tx) => {
        ...
        for (const interest of interestsData) {
          ...
          await tx.insert(employeeInterests).values(insertData);
      ...
      res
        .status(500)
        .json({
          message: error.message || 'Failed to update employee interests',
        });
    }
  }
);
...
router.post(
  '/admin/interests',
  validateRequestBody(createInterestSchema),
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      if (existingInterest) {
        return res
          .status(400)
          .json({ message: 'An interest with this label already exists' });
      }
      ...
      res
        .status(500)
        .json({ message: error.message || 'Failed to create interest' });
    }
  }
);
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || 'Failed to update interest' });
    }
  }
);
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: error.message || 'Failed to delete interest' });
    }
  }
);
...
router.post(
  '/admin/interests/merge',
  validateRequestBody(mergeInterestsSchema),
  async (req: AuthenticatedRequest, res) => {
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.delete(interests).where(inArray(interests.id, mergeIds));
      });
      ...
      res
        .status(500)
        .json({ message: error.message || 'Failed to merge interests' });
    }
  }
);
...
router.post('/groups/:interestId', async (req: AuthenticatedRequest, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: 'Failed to create or join interest group' });
  }
});
...
router.get(
  ...
  async (req: AuthenticatedRequest, res) => {
    ...
    `,
      ...
      res
        .status(500)
        .json({ message: 'Failed to fetch interest group details' });
    }
  }
);
