...
router.post("/types", verifyToken, verifyAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log('Leave microservice: Leave type created successfully:', leaveType.id);
...
router.patch("/types/:id", verifyToken, verifyAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const [updatedType] = await db.update(leaveTypes)
      .set({
        ...req.body,
        updatedAt: new Date()
      })
    ...
    console.log('Leave microservice: Leave type updated successfully:', updatedType.id);  
...
router.delete("/types/:id", verifyToken, verifyAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const entitlements = await db.select({ count: sql<number>`count(*)` })
      .from(leaveEntitlements)
      .where(eq(leaveEntitlements.leaveTypeId, typeId));
      
    if (entitlements[0].count > 0) {
      return res.status(400).json({ error: "Leave type is in use and cannot be deleted" });
    }
    
    await db.delete(leaveTypes)
      .where(eq(leaveTypes.id, typeId));
      
    console.log('Leave microservice: Leave type deleted successfully:', typeId);
    res.status(204).send();
  } catch (error) {
...
router.get("/requests", verifyToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log('Leave microservice: Retrieved', requests.length, 'leave requests');
    res.json(requests);
  } catch (error) {
...
router.post("/requests", verifyToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log('Leave microservice: Leave request created successfully:', request.id);
...
router.patch("/requests/:id", verifyToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const [updatedRequest] = await db.update(leaveRequests)
      .set({
        ...req.body,
        approvedAt: req.body.status === 'APPROVED' ? new Date() : request.approvedAt
      })
      .where(eq(leaveRequests.id, requestId))
      .returning();
      
    console.log('Leave microservice: Leave request updated successfully:', updatedRequest.id);
...
router.get("/policies", verifyToken, verifyAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log('Leave microservice: Retrieved', policies.length, 'leave policies');
    res.json(policies);
  } catch (error) {
    console.error("Leave microservice: Error fetching leave policies:", error);
...
router.post("/policies", verifyToken, verifyAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (existingPolicy) {
      const [updatedPolicy] = await db.update(leavePolicies)
        .set({
          ...req.body,
          updatedAt: new Date(),
          updatedBy: userId
      ...
      console.log('Leave microservice: Leave policy updated successfully:', result.id);
    } 
    // Otherwise, create a new policy
      ...
      const [newPolicy] = await db.insert(leavePolicies)
        .values(validatedData)
        .returning();
        
      result = newPolicy;
      console.log('Leave microservice: Leave policy created successfully:', result.id);
    }
    
    res.status(existingPolicy ? 200 : 201).json(result);
  } catch (error) {
    console.error("Leave microservice: Error creating/updating leave policy:", error);
...
router.get("/holidays", verifyToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (country) {
      queryBuilder = queryBuilder.where(eq(holidays.country, country as string));
    }
...
router.post("/holidays", verifyToken, verifyAdmin, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    console.log('Leave microservice: Holiday created successfully:', holiday.id);
