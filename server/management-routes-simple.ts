...
function calculateSubscriptionStatus(
  ...
  const totalDays =
    period === 'quarter' ? 90 : period === 'year' ? 365 : customDays || 90;
...
router.get(
  '/organizations',
  verifyCorporateAdmin,
  checkPermission('manageOrganizations'),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      if (
        lastPaymentDate &&
        subscriptionPeriod &&
        (!subscribedUsers || !pricePerUserPerMonth)
      ) {
        return res
          .status(400)
          .json({
            error:
              'Subscription requires subscribedUsers and pricePerUserPerMonth',
          });
      }
      ...
      if (existingOrg.length > 0) {
        return res
          .status(400)
          .json({ error: 'Organization slug already exists' });
      }
      ...
      if (lastPaymentDate && subscriptionPeriod) {
        ...
        [newSubscription] = await db
          ...
          .values({
            ...
            totalMonthlyAmount:
              totalMonthlyAmount ||
              (subscribedUsers || 50) * (pricePerUserPerMonth || 10.0), // Calculate total
            isActive: true,
          })
          .returning();
...
router.post(
  '/organizations/:id/reset-password',
  verifyCorporateAdmin,
  checkPermission('manageOrganizations'),
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      if (existingSubscription) {
        return res
          .status(400)
          .json({ error: 'Organization already has an active subscription' });
      }
      ...
      const [newSubscription] = await db
        ...
        .values({
          ...
          totalMonthlyAmount:
            totalMonthlyAmount ||
            (subscribedUsers || 50) * (pricePerUserPerMonth || 10.0),
          isActive: true,
        })
        .returning();
  ...
  async (req, res) => {
    try {
      ...
      if (!currentSubscription) {
        return res
          .status(404)
          .json({ error: 'No subscription found for organization' });
      }
      ...
      const [newSubscription] = await db
        ...
        .values({
          ...
          expirationDate,
          subscribedUsers:
            subscribedUsers || currentSubscription.subscribedUsers || 50,
          pricePerUserPerMonth:
            pricePerUserPerMonth ||
            currentSubscription.pricePerUserPerMonth ||
            10.0,
          totalMonthlyAmount:
            totalMonthlyAmount ||
            (subscribedUsers || currentSubscription.subscribedUsers || 50) *
  ...
  async (req, res) => {
    try {
      ...
      const [subscriptionData] = await db
        ...
        .leftJoin(
          subscriptions,
          eq(subscriptions.id, organizations.currentSubscriptionId)
        )
        .where(eq(organizations.id, Number(id)));
...
router.post(
  '/organizations/:id/subscription/deactivate',
  verifyCorporateAdmin,
  checkPermission('manageOrganizations'),
  async (req, res) => {
    try {
