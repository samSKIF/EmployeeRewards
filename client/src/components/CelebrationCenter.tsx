...
export default function CelebrationCenter() {
  ...
  const getTodaysMessage = () => {
    ...
    const todaysBirthdays = todayCelebrations.filter(
      (c) => c.type === 'birthday'
    );
    const todaysAnniversaries = todayCelebrations.filter(
      (c) => c.type === 'work_anniversary'
    );
  ...
  const filteredExtendedCelebrations = Array.isArray(extendedCelebrations)
    ? extendedCelebrations.filter((event) => {
        const matchesDepartment =
          ...
          event.user?.department === departmentFilter;
        const matchesLocation =
          locationFilter === 'all' || event.user?.location === locationFilter;
        return matchesDepartment && matchesLocation;
      })
    : [];

  const groupedCelebrations =
    filteredExtendedCelebrations?.reduce(
      (groups, event) => {
          ...
          <div className="flex-1">
            ...
            {upcomingCelebrations && upcomingCelebrations.length > 0 && (
              <p className="text-xs text-gray-500 mt-1">
                {upcomingCelebrations.length} upcoming celebration
                {upcomingCelebrations.length !== 1 ? 's' : ''} this week
              </p>
            )}
          </div>
        </div>
                  ...
                  <div className="space-y-6">
                    {(() => {
                      ...
                      return sortedEntries.map(([date, events], index) => {
                        ...
                        const isPast =
                          celebrationDate < today && !isToday(celebrationDate);
                                      ...
                                      <div className="flex items-center gap-2 mt-1">
                                        {event.type === 'birthday' ? (
                                          ...
                                          >
                                            ...
                                            {event.yearsOfService} Year
                                            {event.yearsOfService !== 1
                                              ? 's'
                                              : ''}
                                          </Badge>
                                        )}
