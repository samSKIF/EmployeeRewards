...
const badgeTypes = [
  { value: "excellence", label: "Excellence", icon: <Trophy className="h-4 w-4 text-yellow-500" /> },
  { value: "teamwork", label: "Teamwork", icon: <ThumbsUp className="h-4 w-4 text-blue-500" /> },
  { value: "innovation", label: "Innovation", icon: <Star className="h-4 w-4 text-purple-500" /> },
  { value: "goal", label: "Goal Achievement", icon: <Target className="h-4 w-4 text-green-500" /> },
  { value: "leadership", label: "Leadership", icon: <Award className="h-4 w-4 text-red-500" /> },
];
...
export function RecognitionForm({ onSuccess }: RecognitionFormProps) {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["/api/recognition/received"] });
  ...
  return (
    <Card className="w-full max-w-md mx-auto">
      <CardHeader>
        ...
        <CardDescription>{t("recognition.recognizeDescription")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
