        ...
        <DialogContent className="sm:max-w-md">
                ...
                {selectedOrder.externalRef && (
                  <div className="col-span-2">
                    <h4 className="text-sm font-medium text-gray-500">
                      {selectedOrder.externalRef.includes('http') ? 'Gift Card Link' : 'Order Reference'}
                    </h4>
          ...
          <DialogFooter className="flex space-x-2 justify-end">
            ...
            {selectedOrder && selectedOrder.status !== "completed" && (
              <Button
                onClick={() => updateOrderStatus(getNextStatus(selectedOrder.status))}
                disabled={updateOrderMutation.isPending}
              >
                {updateOrderMutation.isPending ? "Processing..." : (
                  <>
