...
type StatusTypeInput = Omit<
  StatusType,
  'id' | 'isSystem' | 'createdAt' | 'updatedAt' | 'createdBy'
>;
...
const AdminStatusTypes: React.FC = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const response = await apiRequest(
        'GET',
        '/api/employee-status/status-types'
      );
      if (!response.ok) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: t(
          'statusTypes.createSuccess',
          'Status type created successfully'
        ),
        description: t(
          'statusTypes.createSuccessDescription',
          'The new status type has been added to the system.'
        ),
      });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      id: number;
      data: Partial<StatusTypeInput>;
    }) => {
      const response = await apiRequest(
        'PATCH',
        `/api/employee-status/status-types/${id}`,
        data
      );
      if (!response.ok) {
        const errorData = await response.json().catch(() => null);
    ...
    onSuccess: () => {
      toast({
        ...
        description: t(
          'statusTypes.updateSuccessDescription',
          'The status type has been updated.'
        ),
      });
      // Invalidate the status types query to refetch
      queryClient.invalidateQueries({
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast({
        title: t(
          'statusTypes.deleteSuccess',
          'Status type deleted successfully'
        ),
        description: t(
          'statusTypes.deleteSuccessDescription',
          'The status type has been removed from the system.'
        ),
      });
      // Invalidate the status types query to refetch
      queryClient.invalidateQueries({
    ...
    <div className="container mx-auto p-4">
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div>
            <CardTitle>
              {t('statusTypes.title', 'Employee Status Types')}
            </CardTitle>
            <CardDescription>
              {t(
                'statusTypes.description',
                'Configure status indicators that appear next to employee avatars'
              )}
            </CardDescription>
          </div>
              ...
              <TableBody>
                {statusTypes?.length > 0 ? (
                    ...
                    <TableRow key={statusType.id}>
                      ...
                      <TableCell>
                        {statusType.durationDays
                          ? t('statusTypes.days', '{{count}} days', {
                              count: statusType.durationDays,
                            })
                      ...
                      <TableCell>
                        {statusType.isActive ? (
                          <span className="text-green-500">
                            {t('statusTypes.active', 'Active')}
                          </span>
                        ) : (
                          <span className="text-gray-400">
                            {t('statusTypes.inactive', 'Inactive')}
                          </span>
                        )}
                      </TableCell>
                      <TableCell>
                        <div className="flex space-x-2">
                          ...
                          <Button
                            ...
                            disabled={
                              statusType.isSystem || deleteMutation.isPending
                            }
                          >
                  ...
                  <TableRow>
                    ...
                    >
                      {t(
                        'statusTypes.noDataMessage',
                        'No status types found. Click "Add New" to create one.'
                      )}
                    </TableCell>
                  </TableRow>
                )}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            ...
            <DialogDescription>
              {t(
                'statusTypes.dialogDescription',
                'Configure how this status will appear next to employee avatars.'
              )}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  ...
                  min="0"
                  value={
                    formData.durationDays !== null ? formData.durationDays : ''
                  }
                  onChange={(e) =>
                    handleInputChange(
                      'durationDays',
                  ...
                  className="col-span-3"
                  placeholder={t(
                    'statusTypes.indefinitePlaceholder',
