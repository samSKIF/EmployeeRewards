...
const Shop = () => {
  ...
  const featuredItems = filteredProducts.filter(
    (product) => product.points >= 500 && product.points <= 1000
  );

  // Separate luxury items (very expensive products)
  const luxuryItems = filteredProducts.filter(
    (product) => product.points > 1000
  );

  // Regular products (excluding featured and luxury)
  const regularProducts = filteredProducts.filter(
    (product) => product.points < 500
  );
        ...
        <div className="container mx-auto flex justify-between items-center">
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            <h1 className="text-base sm:text-xl font-semibold truncate">
              ...
              <span className="hidden xs:inline">Reward</span> Shop
            </h1>
          </div>
          ...
          <div className="flex items-center space-x-2 sm:space-x-6">
            <div className="bg-white/20 rounded-full px-2 sm:px-4 py-1 sm:py-2 flex items-center">
              ...
              <span className="font-bold text-xs sm:text-base">
                ...
                <span className="hidden xs:inline">{t('shop.points')}</span>
              </span>
            </div>
            <Button
              variant="ghost"
      ...
      <main className="container mx-auto px-3 sm:px-4 py-4 sm:py-8">
              ...
              <div>
                <h3 className="font-medium text-blue-800">
                  ...
                  {selectedBrand.charAt(0).toUpperCase() +
                    selectedBrand.slice(1)}{' '}
                  products
                </h3>
                <p className="text-xs text-blue-600">
                  Products matching your selection from the social feed
                </p>
              </div>
            </div>
            <Button
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-4">
            ...
            <div className="flex flex-wrap gap-1 sm:gap-2 mt-2 md:mt-0">
              <Button
                ...
                style={
                  selectedCategory === 'all' && branding?.primaryColor
              ...
              {categories.map((category) => (
                <Button
                  ...
                  className="text-xs sm:text-sm py-1 px-2 sm:px-3 h-7 sm:h-9"
                  style={
                    selectedCategory === category && branding?.primaryColor
                      ? { background: branding.primaryColor }
                      : undefined
                  }
                >
        ...
        <Tabs defaultValue="all" className="space-y-4 sm:space-y-8">
          <TabsList className="grid w-full grid-cols-4 h-auto p-0.5 sm:p-1">
            <TabsTrigger
              value="all"
              className="py-2 sm:py-3 text-xs sm:text-sm px-1"
            >
              <ShoppingBag className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1 sm:mr-2" />
              <span className="hidden xs:inline">
                {t('shop.allCategories')}
              </span>{' '}
              <span className="hidden sm:inline">{t('shop.rewards')}</span>
            </TabsTrigger>
            <TabsTrigger
              value="experiences"
              className="py-2 sm:py-3 text-xs sm:text-sm px-1"
            ...
            <div className="mb-6">
              ...
              {featuredCategoryProducts.giftCards.length > 0 ? (
                ...
                <Card className="p-8 text-center">
                  ...
                  <p className="text-gray-500">
                    Check back soon for gift card options!
                  </p>
                </Card>
              )}
            </div>
          ...
          <TabsContent value="premium">
            <div className="mb-6">
              ...
              {[...featuredItems, ...luxuryItems].length > 0 ? (
                ...
                <Card className="p-8 text-center">
                  <h3 className="text-xl font-semibold mb-2">
                    No premium rewards available
                  </h3>
                  <p className="text-gray-500">
                    Check back soon for premium reward options!
                  </p>
                </Card>
              )}
            </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="mb-4 md:mb-0 text-center md:text-left">
              ...
              <p className="text-xs sm:text-sm text-gray-400">
                © {new Date().getFullYear()} {t('shop.allRightsReserved')}
              </p>
            </div>
            <div className="flex flex-wrap justify-center md:justify-end gap-3 sm:gap-6">
              <a href="#" className="text-xs sm:text-sm hover:text-white">
                {t('shop.termsAndConditions')}
              </a>
              <a href="#" className="text-xs sm:text-sm hover:text-white">
                {t('shop.privacyPolicy')}
              </a>
              <a href="#" className="text-xs sm:text-sm hover:text-white">
