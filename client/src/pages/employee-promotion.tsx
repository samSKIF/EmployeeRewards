...
export default function EmployeePromotion() {
  ...
  const [promotingEmployee, setPromotingEmployee] = useState<Employee | null>(
    null
  );
  ...
  const savePromotion = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/permissions/${promotingEmployee.id}`,
        {
          method: 'PATCH',
          headers: {
  ...
  const addSite = () => {
    if (
      newSite.trim() &&
      promotingEmployee &&
      !promotingEmployee.allowedSites.includes(newSite.trim())
    ) {
      setPromotingEmployee({
        ...promotingEmployee,
  ...
  const removeSite = (siteToRemove: string) => {
    if (promotingEmployee) {
      setPromotingEmployee({
        ...promotingEmployee,
        allowedSites: promotingEmployee.allowedSites.filter(
  ...
  const addDepartment = () => {
    if (
      newDepartment.trim() &&
      promotingEmployee &&
      !promotingEmployee.allowedDepartments.includes(newDepartment.trim())
    ) {
      setPromotingEmployee({
        ...promotingEmployee,
        allowedDepartments: [
          ...promotingEmployee.allowedDepartments,
          newDepartment.trim(),
        ],
      });
      setNewDepartment('');
  ...
  const removeDepartment = (deptToRemove: string) => {
    if (promotingEmployee) {
      setPromotingEmployee({
        ...promotingEmployee,
        allowedDepartments: promotingEmployee.allowedDepartments.filter(
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Promote employees to admin roles with specific permissions
          </p>
        </div>
      </div>
                ...
                <div className="grid md:grid-cols-2 gap-4 mt-4 pt-4 border-t">
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      ...
                      {(!employee.allowedSites ||
                        employee.allowedSites.length === 0) && (
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      ...
                      {(!employee.allowedDepartments ||
                        employee.allowedDepartments.length === 0) && (
                        <span className="text-sm text-muted-foreground">
                          All departments
                        </span>
                      )}
      ...
      {promotingEmployee && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <CardHeader>
              <CardTitle>
                Promote to Admin - {promotingEmployee.name}{' '}
                {promotingEmployee.surname}
              </CardTitle>
            ...
            <CardContent className="space-y-6">
              ...
              {(promotingEmployee.adminScope === 'site' ||
                promotingEmployee.adminScope === 'hybrid') && (
                <div>
                    ...
                    <Select value={newSite} onValueChange={setNewSite}>
                      ...
                      <SelectContent>
                        {allSites
                          .filter(
                            (site) =>
                              !promotingEmployee.allowedSites.includes(site)
                          )
                          .map((site) => (
              ...
              {(promotingEmployee.adminScope === 'department' ||
                promotingEmployee.adminScope === 'hybrid') && (
                <div>
                  <Label>
                    Managed Departments (Multiple Selection Allowed)
                  </Label>
                  <div className="flex flex-wrap gap-2 mb-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        {allDepartments
                          .filter(
                            (dept) =>
                              !promotingEmployee.allowedDepartments.includes(
                                dept
                              )
                          )
                          .map((dept) => (
              ...
              <div className="flex justify-end gap-2">
                <Button
                  variant="outline"
                  onClick={() => setPromotingEmployee(null)}
                >
                  Cancel
                </Button>
                <Button
                  onClick={savePromotion}
                  disabled={promotingEmployee.adminScope === 'none'}
                >
