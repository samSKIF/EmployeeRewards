...
export default function AdminFeaturedPosts() {
  ...
  const { data: featuredPosts = [], refetch: refetchFeaturedPosts } = useQuery<
    FeaturedPost[]
  >({
  ...
  const handlePinPost = (postId: number) => {
    const nextOrder =
      Math.max(...featuredPosts.map((p) => p.pinnedOrder || 0), 0) + 1;
    ...
    <div className="container mx-auto p-6 space-y-8">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">
            Configure what content appears in the top section of the Spaces
            discovery page
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={setSelectedDisplayMode}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="engagement">
                    Most Engaging (48 hours)
                  </SelectItem>
                  <SelectItem value="latest_from_spaces">
                    Latest from Specific Spaces
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setMaxPosts(parseInt(value))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          ...
                          setSelectedSpaces(
                            selectedSpaces.filter((id) => id !== channel.id)
                          );
                        }
                      }}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Posts currently displayed in the Spaces discovery page (
            {featuredPosts.length} of {maxPosts} slots used)
          </CardDescription>
        </CardHeader>
        <CardContent>
          {featuredPosts.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              {selectedDisplayMode === 'pinned' &&
                ' Pin some posts below to get started.'}
              {selectedDisplayMode === 'engagement' &&
                ' Posts with high engagement will appear automatically.'}
              {selectedDisplayMode === 'latest_from_spaces' &&
                ' Select spaces above to show their latest posts.'}
            </div>
          ) : (
