      ...
      <div className="max-w-7xl mx-auto">
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="mt-4">
                ...
                <h3 className="text-2xl font-bold">{stats?.pointsBalance?.toLocaleString() || 0}</h3>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="mt-4">
                ...
                <h3 className="text-2xl font-bold">{(stats?.posts || 0) + (stats?.comments || 0)}</h3>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="points" className="mb-8">
          <TabsList className="w-full max-w-md mb-4">
            <TabsTrigger value="points" className="flex-1">Points Analytics</TabsTrigger>
          ...
          <TabsContent value="points" className="bg-white p-6 rounded-lg shadow">
            <CardHeader className="px-0 pt-0">
              <CardTitle>Points Distribution Over Time</CardTitle>
              <CardDescription>Points earned and spent through the platform.</CardDescription>
            </CardHeader>
          ...
          <TabsContent value="engagement" className="bg-white p-6 rounded-lg shadow">
            <CardHeader className="px-0 pt-0">
              <CardTitle>Platform Engagement</CardTitle>
              <CardDescription>Social interactions by category.</CardDescription>
            </CardHeader>
            <CardContent className="px-0 pb-0">
              <div className="h-[350px]">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <TabsContent value="rewards" className="bg-white p-6 rounded-lg shadow">
            <CardHeader className="px-0 pt-0">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="space-y-4">
                  {!stats?.recentRecognitions || stats.recentRecognitions?.length === 0 ? (
                    <p className="text-gray-500 text-center py-4">No recent recognitions</p>
                  ) : (
                    stats.recentRecognitions.map((recognition, index) => (
                      <div key={index} className="flex items-start space-x-4">
                        ...
                        <div>
                          <p className="font-medium">{recognition.fromUser?.name} → {recognition.toUser?.name}</p>
                          <p className="text-sm text-gray-500">{recognition.message}</p>
                          <div className="mt-1 text-xs text-gray-400">{new Date(recognition.createdAt).toLocaleDateString()}</div>
                        </div>
                      </div>
                    ))
                  )}
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Users with highest points this month.</CardDescription>
            </CardHeader>
            <CardContent>
              {isLoading ? (
                <div className="space-y-2">
                    ...
                    stats.topPerformers.map((performer, index) => (
                      <div key={index} className="flex items-center">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-gray-500">{performer.department || 'No department'}</p>
                        </div>
