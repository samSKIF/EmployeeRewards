...
async function generateRealisticChannelContent() {
  ...
  try {
    ...
    for (const channel of channels) {
      const channelTypeImages = coverImages[channel.channelType] || coverImages['interest'];
    ...
    for (const channel of channels) {
      ...
      if (channel.channelType === 'department' && channel.allowedDepartments?.length > 0) {
        ...
        templates = postTemplates['department'][dept] || postTemplates['interest'];
      } else if (postTemplates[channel.channelType]) {
        templates = postTemplates[channel.channelType];
      }
