  ...
  try {
    ...
    for (const location of locations) {
      try {
        ...
        await pool.query(`
          INSERT INTO locations (name, organization_id, city, country)
          VALUES ($1, 1, $2, $3)
          ON CONFLICT (name, organization_id) DO NOTHING
    ...
    // Verify the data
    const departmentCount = await pool.query('SELECT COUNT(*) FROM departments WHERE organization_id = 1');
    const locationCount = await pool.query('SELECT COUNT(*) FROM locations WHERE organization_id = 1');
